...
for iyoku_file_input_name in iyoku_files:
	...
	with open(iyoku_file_input_name, 'r', encoding='utf-8') as iyoku_file:
		iyoku_unit_byID = {}
		for line in iyoku_file:
			if(len(re.findall('1023456', line)) != 0):
				continue
			if(len(re.findall('所属組織', line)) != 0):
				continue
			#print(logfile_line)
			iyoku_file_data = line[:-1].split(',')
			studentID = iyoku_file_data[2]

			#複数回答の場合は初めの回答を採用する処理
			if studentID not in iyoku_unit_byID:
				# 意欲
				if(len(re.findall('この授業の試験範囲の内容に取り組みたい', line)) != 0):
					iyoku_unit_byID[studentID] = '1'
				elif(len(re.findall('余裕があればこの授業の高度な内容に取り組みたい', line)) != 0):
					iyoku_unit_byID[studentID] = '2'
				elif(len(re.findall('この授業に関連する高度な内容に取り組みたい', line)) != 0):
					iyoku_unit_byID[studentID] = '3'
				else:
					iyoku_unit_byID[studentID] = '0'
...
for iyokuFinal in sorted(iyoku_byID.keys()):
	...
	for m in range(1, len(iyoku_out_data)):
		if iyoku_out_data[m] == '-':  # 未回答のデータをチェック
			iyoku_out_data[m] = '0'   # 未回答を0に置換
