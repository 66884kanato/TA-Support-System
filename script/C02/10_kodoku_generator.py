...
#	print(kodoku_file_input_name) # debug
#	print(kodoku_file_number) # debug
...
# 無関係の設問のカット
		...
		kodoku_file_line = kodoku_file_line.replace(',受講上の懸念はない', '-')
		kodoku_file_line = kodoku_file_line.replace(',コンピュータの環境について、速度やアプリケーションの動作などの受講上の懸念がある', '-')

#		print(kodoku_file_line)
		kodoku_file_data = kodoku_file_line[:-1].split(',')
		if(len(kodoku_file_data)>12):
			kodoku_unit_ByID[kodoku_file_data[2]] = kodoku_file_data[12]
		else:
			kodoku_unit_ByID[kodoku_file_data[2]] = '-'
#		print(kodoku_unit_ByID[kodoku_file_data[2]])
