...
//ipアドレス94-102（前の席101-109まで)
for(var i=0; i<9; i++){

    var k = i + 74; //ipアドレス94-102は配列ipの74番目以降
    seat_inf[i] = [];// 各seat_inf[i]を空の配列として初期化(これによって2次元配列化)
    var ip_inf = ip[k][0].split(".");
    var seat_ip = parseInt(ip_inf[3]);

    seat_inf[i][0] = (seat_ip + ip_94_102).toString(); //IPアドレスの３番目だけをseat_infの0列目に格納
    //ここからはクエリパラメータと同じ（scoreより前）
    seat_inf[i][1] = ip[k][1]; //seat_infの1列目にstudent_idを格納(いない場合は0が格納)
    seat_inf[i][2] = ta_id; //seat_infの2列目にta_idを格納

    for(var j=0; j<que_all.length; j++){

        if(seat_inf[i][1]==que_all[j][0]){ //que_all.csv側の学籍番号とip.csv側の学籍番号が一致したら
            //seat_infの3列目に重要度を格納
...
//ipアドレス20-92(93は先生のアドレスだから無視)
var k = i; //iを格納しておく(i=9)
for(var i; i<count_ip-2; i++){ //i=9から

    var l = i - k; //今度はipの０番目からipアドレスをとってくる（l=0から)
    seat_inf[i] = []; // 各seat_inf[i]を空の配列として初期化(これによって2次元配列化)
    var ip_inf = ip[l][0].split(".");
    var seat_ip = parseInt(ip_inf[3]);

    seat_inf[i][0] = (seat_ip + ip_to_seat).toString(); //IPアドレスの３番目だけをseat_infの0列目に格納
    //ここからはクエリパラメータと同じ（score,koekake以外）
    seat_inf[i][1] = ip[l][1]; //seat_infの1列目にstudent_idを格納(いない場合は0が格納)
    seat_inf[i][2] = ta_id; //seat_infの2列目にta_idを格納
