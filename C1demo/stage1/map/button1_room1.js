    ...
    while(now_ip<=end_ip[i]){
      ...
      if(seat_inf[seat_inf_index][1]!=0){ //その座席に座っているなら
        ...
        if(seat_inf[seat_inf_index][1]!=student_id){ //声掛け学生と一致しないなら
          ...
          seatButton.dataset.level = seat_inf[seat_inf_index][3];
          ...
          seatButton.addEventListener('click', function(event){
            ...
            window.location.href = `../php/taio1.php?student_id=${params1}&ta_id=${params2}&level=${params3}&completion=${params4}&again=${params5}&skip=${params6}&kyosyu=${params7}&score=${params8}&koekake=${params9}&quest=${params10}&stage=${params11}`
          });
      ...
      // 新しい座標をボタンに設定
      seatButton.style.left = `${move_seat_x}px`; //第1演習室なのでrightから
